...
export default function PlanOverview() {
  ...
  const standardCourseLabelIds = standardCourseTabs.map(tab => tab.id).filter(id => id !== 'ALL')
  const customCourseLabels = [...new Set((plan?.courses || []).flatMap(course => course.labels || []))]
  ...
  return <div className="plan-overview-page">
    <div className="page-header plan-overview-header"><h1 className="plan-overview-title">{plan.logo_url || plan.logo ? <img src={plan.logo_url || plan.logo} alt="" /> : <span className="plan-overview-logo-fallback">{plan.name?.charAt(0).toUpperCase() || '?'}</span>}<span>{plan.name}</span></h1><div className="plan-action-panel"><button className="btn btn-secondary btn-sm icon-button" title="Learning plan overview" aria-label="Learning plan overview" onClick={() => setShowOverview(true)}><WorkspaceIcon name="info" /></button><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search courses..." aria-label="Search courses" /><button className={`btn btn-secondary btn-sm icon-button ${labelFilters.length ? 'active' : ''}`} title="Sort and filter courses" aria-label="Sort and filter courses" onClick={() => setShowSortFilter(true)}><WorkspaceIcon name="sort" /></button><button className="btn btn-secondary btn-sm" onClick={() => navigate(`/plans/${planId}/ai-requests`)}><WorkspaceIcon name="ai" />AI requests</button><div className="add-course-group"><button className="btn btn-secondary btn-sm" onClick={() => setShowManual(true)}><WorkspaceIcon name="manual" />Manual</button><button className="btn btn-secondary btn-sm" onClick={() => setShowAi(true)}><WorkspaceIcon name="ai" />AI</button></div></div></div>
    <div className="page-header course-toolbar"><h4>Courses <span className="badge badge-green">{plan.courses?.length || 0}</span></h4></div>
    <div className="label-tabs" role="tablist" aria-label="Filter courses by standard label">{standardCourseTabs.map(tab => <button key={tab.id} className={courseLabelTab === tab.id ? 'active' : ''} onClick={() => setCourseLabelTab(tab.id)}>{tab.label} <span>{tab.id === 'ALL' ? plan.courses?.length || 0 : plan.courses.filter(course => course.labels?.includes(tab.id)).length}</span></button>)}</div>
    ...
    {showSortFilter && <><div className="drawer-overlay" onClick={() => setShowSortFilter(false)} /><aside className="drawer"><div className="drawer-header"><h2>Sort and filter courses</h2><button className="btn btn-secondary btn-sm" onClick={() => setShowSortFilter(false)}>×</button></div><div className="drawer-body"><section className="workspace-filter-section"><label>Filter by custom labels</label>{customCourseLabels.length ? customCourseLabels.map(label => <label className="filter-checkbox" key={label}><input type="checkbox" checked={labelFilters.includes(label)} onChange={() => setLabelFilters(current => current.includes(label) ? current.filter(item => item !== label) : [...current, label])} />{label}</label>) : <p className="tile-date">No custom course labels yet.</p>}</section><div className="material-select"><label>Sort courses</label><div className="sort-toggle" role="group" aria-label="Sort courses"><button className={sortBy === 'updated' ? 'active' : ''} onClick={() => setSortBy('updated')}>Recently updated</button><button className={sortBy === 'name' ? 'active' : ''} onClick={() => setSortBy('name')}>Name</button></div></div></div><div className="drawer-footer"><button className="btn btn-secondary" onClick={() => { setLabelFilters([]); setSortBy('updated') }}>Reset</button><button className="btn btn-primary" onClick={() => setShowSortFilter(false)}>Apply</button></div></aside></>}
